...
class BigQuery:
    ...
    def get_query(self, table: str = None) -> str:
        """
        ...
        QUERY_SQL = \
            {
                ...
                "sql_customer"  : f"""
                    SELECT
                        ...
                        TBAS.state,
                        CASE
                            WHEN TBCS.gender = "F" THEN "Female"
                            WHEN TBCS.gender = "M"THEN "Male"
                            ELSE "Unknow"
                        END AS gender
                    FROM
                ...
                "sql_sales_forecast"  : f"""
                    SELECT
                        ...
                        ROUND(prophet, 2) AS prophet,
                        ROUND(holt_winters, 2) AS holt_winters
                    FROM
                        `gcp-mts-pf.production.tb_sales_forecast`
                    """
            }

        return QUERY_SQL[table] if table in QUERY_SQL else None


    def read_bq(self, sql_script: str) -> list[list]:
        """
            ...
            ----------
            sql_script : str
                The SQL script to execute. Legacy SQL is disabled (standard SQL is used).
...
if __name__ == '__main__':
    ...
    for query in ['sql_sales_forecast']:
        result = bq.read_bq(
            sql_script=query
        )
        print(result.head())