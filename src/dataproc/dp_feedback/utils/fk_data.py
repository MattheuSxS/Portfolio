...
class FkFeedback:
    ...
    def generate_fake_feedbacks(self, num_feedbacks:int = 10, category:str = None) -> list[dict]:
        feedbacks = []
        for _ in range(num_feedbacks):
            ...
            if feedback["type"] == "Product":
                feedback.update({
                    "product_name": None,
                    "brand_name": None,
                    "size": random.choice(["S", "M", "L", "XL", "One Size"]) if category in ["Clothing", "Toys"] else None,
                    "color": self.fake.color_name() if category in ["Clothing", "Furniture"] else None
                })
...
if __name__ == "__main__":
    ...
    feedbacks = test.generate_fake_feedbacks(1, "Clothing")
