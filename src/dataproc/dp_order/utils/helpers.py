...
def _udf_functions() -> list:
    ...
    """
    generate_random_date_udf = F.udf(_generate_random_date, TimestampType())
    generate_peak_season_multiplier_udf = F.udf(_generate_peak_season_multiplier, FloatType())
    generate_order_id_udf = F.udf(_generate_order_id, StringType())
...
def generate_fake_purchases(df_customers, df_products, VAR_NUM_PURCHASES):
    ...
    region_stats = region_stats.join(
        df_products_indexed.groupBy("region").agg(F.count("*").alias("total_products")),
        on="region",
        how="left"
    ).fillna(0)
    ...
    total_customers = purchases_per_region.agg(F.sum("customer_count")).first()[0]
    ...
    df_base = df_base.withColumn(
        "cust_idx",
        F.floor(F.rand() * F.col("total_customers"))
    ).withColumn(
        "prod_idx",
        F.floor(F.rand() * F.col("total_products"))
...
def generate_fake_orders(df_base):
    ...
    df_base = \
        df_base.withColumn("purchase_id", generate_order_id_udf()) \
            ...
            .withColumn("quantity", F.expr("CAST(FLOOR(RAND() * 10 + 1) AS INT)")) \
            .withColumn("normal_processing_time", F.expr("CAST(FLOOR(RAND() * 10 + 1) AS INT)")) \
            .withColumn("expedited_processing_time", F.expr("CAST(FLOOR(RAND() * 5 + 1) AS INT)")) \
            .withColumn("peak_season_multiplier", generate_peak_season_multiplier_udf()) \
            .withColumn(
            "order_status",
                F.when(F.rand() < 0.80, "completed")
                .when(F.rand() < 0.90, "processing")
                .otherwise("cancelled")) \
            .withColumn("discount_applied", F.expr("ROUND(RAND() * 20, 2)")) \
            .withColumn("final_price", F.expr("ROUND((price * quantity) - discount_applied, 2)"))
