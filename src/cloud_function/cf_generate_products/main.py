    ...
    def generate_products(self, num_products=20):
        ...
        for _ in range(num_products):
            ...
            products.append({
                ...
                "created_at": self.fake.date_time_between(start_date='-2y', end_date='now').strftime('%Y-%m-%d')
            })
        return products
        ...
        for product in products:
            for location in locations:
                inventory.append({
                    ...
                    "last_restock": self.fake.date_time_between(start_date='-6m', end_date='now').strftime('%Y-%m-%d'),
        ...
        for product in random.sample(products, min(20, len(products))):
            for location in random.sample(locations, min(10, len(locations))):
                ...
                processing_data.append({
                    ...
                    "last_updated": self.fake.date_time_between(start_date='-60d', end_date='now').strftime('%Y-%m-%d')
                })
