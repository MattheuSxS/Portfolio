...
resource "null_resource" "push_sentiment_analysis_image" {
    ...
    triggers = {
        script_hash = md5(file("${var.run_path_all_files}/sentiment_analysis/Dockerfile"))
    }

    provisioner "local-exec" {
        command = <<EOT
            cd ${var.run_path_all_files}/sentiment_analysis && \
...
resource "null_resource" "push_logistream_solutions_report_image" {
    ...
    triggers = {
        script_hash = md5(file("${var.run_path_all_files}/market_research/Dockerfile"))
    }

    provisioner "local-exec" {
        command = <<EOT
            cd ${var.run_path_all_files}/market_research && \
