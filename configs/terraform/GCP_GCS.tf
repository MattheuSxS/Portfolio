...
resource "google_storage_bucket_object" "cf_sales_forecast_files" {
    name            = "${var.cf_sales_forecast}/src/index.zip"
    bucket          = "${local.bkt_cf_portfolio}"
    source          = data.archive_file.cf_path_sales_forecast_files.output_path
...
# resource "null_resource" "airflow_variables_v1" {
#     depends_on = [google_composer_environment.portfolio-composer]

#     provisioner "local-exec" {
#         when    = create
#         command = <<EOT
#             jq -s 'add' ../pipe/dev_env/*.json > ../pipe/dev_env/variables-all.json
#             gsutil cp ../pipe/dev_env/variables-all.json gs://${local.bkt_airflow}/data/variables-all.json
#             rm -f ../pipe/dev_env/variables-all.json
#             gcloud composer environments run ${var.composer_name} \
#                 --location="us-east1" \
#                 variables import \
#                 -- gs://${local.bkt_airflow}/data/variables-all.json
#         EOT
#     }
# }
